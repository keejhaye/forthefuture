module.exports = function(io,Statistics){
	...
	Conversations = {
	    ...
	    updateConversationData : function(data, user_id, now){
	        Conversations.logs[data.conversation_id].push({"time" : now, "message":'Conversation is assigned to :' + Users[user_id].username})
	    ...
	    unmapActiveConversation : function(data){
	        ...
	        if(Users[data.user_id] != undefined){
	            ...
	            Conversations.logs[data.conversation_id].push({"time": gmdate('Y-m-d H:i:s') , "message":'Conversation unmapped from user:' + Users[data.user_id].username})
	            if(conv_index != -1){
	    ...
	    tryToFetchNewConversation : function(user_id){
	        ...
	        if(!_lodash.isEmpty(conversation)){
                ...
                if(Conversations.list[conversation.conversation_id] != undefined){
                	...
                	Conversations.logs[conversation.conversation_id].push({"time":now, "message":'Conversation is assigned to user:' + Users[user_id].username})
	    ...
	    queueConversation : function(data, unmapped){
        	...
        	// if(data.message_id != undefined)
	     	...
	     	Conversations.logs[data.conversation_id].push({"time":now, "message":'Conversation is queued'});
	     	message_ids = Object.keys(Conversations.list[data.conversation_id].messages)
	    ...
	    handle_blacklisted_subscriber_messages : function(data){
	    	if(data.latest_message_ids.length > 0){
	    		Models.TblMessages.handleMessages(data.latest_message_ids, data.user_id, data.fullname, data.handle_time);
	    	}
	    }
	}
