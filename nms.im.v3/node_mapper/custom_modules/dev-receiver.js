module.exports = function(io,Statistics,redisClient) {
    ...
    receive_dev_process = function(info){
        ...
        if(info.action === 'update_conversation_status'){
            update_conversation_status(info)
        }
    }
    ...
    update_conversation_status = function(data){
        if(Conversations.list[data.conversation_id] != undefined){
            Conversations.list[data.conversation_id].status = data.status
            Status.update_conversations('all', Conversations.list[data.conversation_id]);

            conversation_data = Conversations.list[data.conversation_id]
            if(Conversations.list[data.conversation_id].service_id != undefined){
                data.service_id = conversation_data.service_id
                if(Services[data.service_id] != undefined && Services[data.service_id].queue[data.conversation_id] == undefined){
                    Services[data.service_id].queue[data.conversation_id] = {
                        id: data.conversation_id, 
                        conversation_id: data.conversation_id, 
                        service_id: data.service_id, 
                        persona_id: conversation_data.persona_id, 
                        status: 'pending',
                        date: conversation_data.wait_start }
                }
                else if(Services[data.service_id] != undefined && Services[data.service_id].queue[data.conversation_id] != undefined){
