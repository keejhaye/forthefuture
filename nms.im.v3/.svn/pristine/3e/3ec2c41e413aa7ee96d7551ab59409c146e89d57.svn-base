module.exports = function(io,Statistics) {
    ...
    chat.on('connection', function(client_socket) {
        ...
        client_socket.on('user_unmap_conversation', function(cid){
            console.log('conversation ['+ cid +'] has been closed by user ['+ Users[client_socket.user_data.id].username +']')
            data = Conversations.list[cid]
            logs = Conversations.logs[cid]

            if(Conversations.logs[cid] != undefined){
                Conversations.logs[cid].push({"time":gmdate('Y-m-d H:i:s'), "message":'Conversation has been closed by user ' + Users[data.user_id].username + ' | last message type: ' + data.last_message_type})
                logs = Conversations.logs[cid]
            }
