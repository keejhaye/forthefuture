...
module.exports = function(sequelize, DataTypes) {
	return sequelize.define('tbl_logged_in_users', {
		...
		classMethods : {
			...
			totalChatting : function(services, callback){
				service_query = ""
				if(services != undefined){
					service_query = " LEFT JOIN tbl_user_service us ON us.user_id = l.user_id WHERE us.service_id IN ("+services+")"
					query_string = "SELECT COUNT(DISTINCT l.id) as chatting FROM tbl_logged_in_users l" + service_query + "AND l.status = 'chat'"
				}else{
						query_string = "SELECT COUNT(DISTINCT l.id) as chatting FROM tbl_logged_in_users l WHERE l.status = 'chat'"
				}
