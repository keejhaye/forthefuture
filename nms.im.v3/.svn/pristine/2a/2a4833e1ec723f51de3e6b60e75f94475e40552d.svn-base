  ...
  Config = require('../config.js')
    ...
    getCustomerStatistics : function(keys, data){
      ...
      Models.TblLoggedInUsers.totalOnline(services, function(rows){
       ...
       Models.TblLoggedInUsers.totalChatting(services, function(rows){
        Statistics.updateCustomerHeaderStatistics({type: 'chatting_count', total:rows[0].chatting})
      })
      ...
      Models.TblConversations.getStatsUpdate(function(rows){
        ...
        Models.TblLoggedInUsers.totalOnline(undefined, function(rows){
          redis_client_statistics.set("online_count", rows[0].online)
           ...
           Models.TblLoggedInUsers.totalChatting(undefined, function(rows){
          redis_client_statistics.set("chatting_count", rows[0].chatting)
        })
        ...
        _lodash(rows).forEach(function(service){
          ...
          redis_client_statistics.set("chatting_count:"+service.id, service.chatting)
        })
