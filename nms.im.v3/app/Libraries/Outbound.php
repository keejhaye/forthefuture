<?php namespace App\libraries {
    ...
    class Outbound {
      ...
      public function send($data, $message, $user = null, $outbound_time = null, $inbound_data = null, $image = array(), $path="", $expiry="") {
        ...
        $billed["operatorId"] = $user->username;
		    ...
		    $aggregatorResponse->meta = json_encode($this->format_outbound($inbound_data, $message, $image, $path,$inbound_data->conversation_id, $outbound_additional_info, $outbound->id));
		...
		protected function format_outbound($inbound, $outbound, $image, $path, $conversation_id, $additional_info = array(), $message_id){
    ...
    $meta["attachment"] = "{".$file."}";
			...
			if($inbound->additional_info != null && $inbound->additional_info != ""){
        		...
        		\Redis::publish('send-targeturl', json_encode($this->array_push_assoc($meta, 
        			[
        				"id" => $message_id,
        				"target_url" => $inbound->aggregator_url
    				]
        			) , JSON_PRETTY_PRINT));
				return $meta;	
			}else{
		...
		protected function array_push_assoc($array, $wholeArray){
			foreach ($wholeArray as $key => $value) {
				$array[$key] = $value;
			}
			return $array;
		}
